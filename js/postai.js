...
function ChucklePostAI(AI_option) {
  ...
  function MAIN(AI_option) {
    ...
    if (
      AI_option.eliminate &&
      AI_option.eliminate.length &&
      AI_option.eliminate.some((item) => currentURL.includes(item))
    ) {
    ...
    if (
      AI_option.whitelist &&
      AI_option.whitelist.length &&
      !AI_option.whitelist.some((item) => currentURL.includes(item))
    ) {
    ...
    if (!AI_option.auto_mount && AI_option.el) {
      targetElement = document.querySelector(
        AI_option.el ? AI_option.el : "#post #article-container",
      );
    } else {
    ...
    // 获取文章标题
    const post_title = document.querySelector(AI_option.title_el)
      ? document.querySelector(AI_option.title_el).textContent
      : document.title;
    ...
    const interface = {
      name: themeAI.ai_name || "clover-AI",
      introduce: themeAI.ai_introduce || "我是文章辅助AI: clover-AI，一个基于deepseek的强大语言模型，有什么可以帮到您？😊",
      version: themeAI.ai_version || "deepseek-v4-pro",
      button: (themeAI.buttons && themeAI.buttons.length) ? themeAI.buttons : ["介绍自己😎", "来点灵感💡", "生成AI简介🤖"],
      ...AI_option.interface,
    };
      ...
      if (elapsed >= speed) {
        ...
        if (i < ai_str_length - 1) {
          ...
          if (delay === character_speed) {
            document.querySelector(".ai-explanation .ai-cursor").style.opacity =
              "0";
          }
          if (i === ai_str_length - 1) {
            observer.disconnect();
    ...
    function getText(element) {
      ...
      const excludeClasses = AI_option.exclude
        ? AI_option.exclude
        : ["highlight", "Copyright-Notice", "post-ai", "post-series"];
      ...
      if (element.innerText) {
        ...
        for (let node of element.childNodes) {
          if (node.nodeType === Node.TEXT_NODE) {
            ...
            let hasExcludeTag = excludeTags.includes(
        ...
        for (let selector of commonSelectors) {
          ...
          if (fallbackElement && fallbackElement !== element) {
            ...
            const fallbackContent =
              fallbackElement.innerText || getText(fallbackElement);
    ...
    function extractString(str, totalLength = 1000) {
      ...
      const lastPart = str.substring(
        str.length - Math.floor(totalLength * 0.5),
      );
    ...
    function insertCSS() {
      ...
      AI_option.additional_css &&
        (styleElement.textContent += AI_option.additional_css);
